...
# cd src1
# git clone https://github.com/GeoLabs/ets-ogcapi-processes120.git
# cd ..
#
...
#
# docker build . -f src1/ets-ogcapi-processes120/Dockerfile -t teamengine/ogcapi-processes:latest
#
...
RUN apt-get update && \
    ...
    mvn -f /home/app/src1/ets-ogcapi-processes120/pom.xml clean install && \
...
COPY --from=build /home/app/src1/ets-ogcapi-processes120/target/ets-ogcapi-processes120-*-aio.jar /root
COPY --from=build /home/app/src1/ets-ogcapi-processes120/target/ets-ogcapi-processes120-*-ctl.zip /root
COPY --from=build /home/app/src1/ets-ogcapi-processes120/target/ets-ogcapi-processes120-*-deps.zip /root
...
RUN cd /root && \
    ...
    apt-get update && \
    apt-get install -y $BUILD_DEPS && \
    unzip  -o teamengine*.war -d /usr/local/tomcat/webapps/teamengine && \
    unzip  -o teamengine-*common-libs.zip -d /usr/local/tomcat/lib && \
    unzip  -o teamengine-console-*-base.zip -d /root/te_base && \
    unzip  -o ets-ogcapi-processes120-*-ctl.zip -d /root/te_base/scripts && \
    unzip  -o ets-ogcapi-processes120-*-deps.zip -d /usr/local/tomcat/webapps/teamengine/WEB-INF/lib && \
    rm -f *zip *war && \
    apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false $BUILD_DEPS && \
